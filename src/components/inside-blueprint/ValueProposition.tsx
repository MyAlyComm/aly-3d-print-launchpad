...
export const ValueProposition = () => {
  ...
  return <section className="py-20 bg-gradient-to-b from-white to-primary/5 overflow-hidden">
      <div className="container px-4 mx-auto">
        <div className={`max-w-3xl mx-auto text-center mb-16 transition-all duration-700 transform ${isInView ? 'translate-y-0 opacity-100' : 'translate-y-10 opacity-0'}`}>
          ...
          <p className="text-gray-600 text-lg">Stop wasting time with trial and error. Get the exact roadmap, systems and AI strategies 
I used to build my successful business.</p>
        </div>
        <div className="grid md:grid-cols-3 gap-8">
          {benefits.map((benefit, index) => <Card key={index} className={`group hover:shadow-lg transition-all duration-500 cursor-pointer overflow-hidden transform hover:-translate-y-2 ${isInView ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} style={{
